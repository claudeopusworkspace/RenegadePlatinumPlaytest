...
- **Location**: Wayward Cave B1F map 285 (33, 7) facing down, on the level-2 bridge row just past the row-6 ramp chain. Save state: `session47_wayward_b1f_post_obj3_rare_candy`. obj:3 Pokéball just picked up (Rare Candy, in bag). Full team healed. 3 Repels in bag (one active).
...
- **Notable items**: Explorer Kit, **Hard Stone**, Dawn Stone, **Dusk Stone**, Wise Glasses, **TM74 Gyro Ball**, **TM32 Double Team**, **TM85 Dazzling Gleam** (new session 29, Mira quest reward — earmarked for Togepi post-evolution), **PP Up**, TM16 Light Screen, TM33 Reflect, TM73 Thunder Wave, **Focus Band**, **Grip Claw** (new, Wayward B1F obj:1), Oval Stone, Fire Stone, Sun Stone, Never-Melt Ice, **Max Ether** (new, Wayward B1F obj:2). Bag: 7 Super Potions, 4 Repels, **0 Revival Herbs** (restock before Mt. Coronet).
- **Wayward Cave status**: Main branch complete (trainers + Mira). B1F east chamber: obj:1 (Grip Claw), obj:2 (Max Ether), and **obj:3 (Rare Candy, session 47)** all picked up. Session 42: slope-climb bug (wrong gear address/encoding) fixed; we can now reach the level-2 bridge row cleanly. Session 43: row-6 ramp chain solved — BFS ramp+3 edge (emitted when ramp+4 is blocked by a same-dir ramp/wall/NPC; engine auto-truncates), plus 3D-BFS momentum-preservation across level transitions. BUG-046 resolved as side-effect. Session 45: **BUG-048 Gap 1 fixed**. **Session 47: BUG-048 Gap 2 fixed** — direction-agnostic fast-bike momentum (verified by `spike_bike_snake_phase1.py` EXP G + EXP F: gear stays FAST through turns, post-turn tiles already at 4f/tile cadence). BFS state simplified to `(x, y, m)`; `_bike_ramp_segment` and `_step_needs_bike` now span direction changes; `_execute_path` drives multi-direction segments via `drive_bike_subsegments` (advance_frames_until + final_buttons handoff per MelonMCP issues #10/#11/#12). obj:3 Pokéball reached from session42 via the new turn-into-ramp executor. warp:0 to map 284 (43, 38) **still unreachable** — different chamber geometry past the row-6 chain, separate problem. One Pokéball at (57, 53) still puzzle-gated — skip unless easy.
...
- **Next**: warp:0 at (43, 38) is still unreachable from session47 — the south end of the east chamber is a separate geometry problem (Manhattan 41, behind whatever cuts off the south-eastern corner of the chamber). Skip it for now or open a new bug entry once we understand the missing piece. Otherwise: return to Route 206 (310, 608), clear the reachable under-bridge Pokéballs + Berry Soils, south through the gate house to Route 207, clear Psychic Arianna, enter Mt. Coronet.

See GAME_HISTORY.md for full details (defeated trainers, story progress, box contents, items).

## Test Suite

Integration tests live in `tests/` (529 tests across 42 files). Require at least one running emulator with the ROM loaded. Legacy DeSmuME tests in `tests/legacy/` are excluded by default.

...
- **Location**: Wayward Cave B1F map 285 (25, 6) facing right, level 2 bridge row just east of the first E-bound row-6 bike ramp — mid-puzzle for the next Pokéball. Save state: `session42_wayward_b1f_first_ramp_approach`. Full team healed. 3 Repels in bag (one active).
...
- **Wayward Cave status**: Main branch complete (trainers + Mira). B1F east chamber: obj:1 (Grip Claw) and obj:2 (Max Ether) picked up session 41. Session 42: slope-climb bug (wrong gear address/encoding) fixed; we can now reach the level-2 bridge row cleanly. Session 43: row-6 ramp chain solved — BFS ramp+3 edge (emitted when ramp+4 is blocked by a same-dir ramp/wall/NPC; engine auto-truncates), plus 3D-BFS momentum-preservation across level transitions. BUG-046 resolved as side-effect. Session 45: **BUG-048 Gap 1 fixed** — `_bike_ramp_edges` now emits a CHAIN_THROUGH edge at approach+10 (= chain-ramp+5, +1 over ground-approach due to in-flight chain-entry) alongside FAR_SHORT at ramp+3; executor holds through the chain animation via a split release-target. East chamber entry at (38, 6) now pathable. obj:3 Pokéball (33, 8) reachable from session42 via FAR_SHORT (unchanged). warp:0 to map 284 (43, 38) **still unreachable** — **BUG-048 Gap 2** open: the chamber-interior `bike_ramp_W` has <RUNWAY_TILES straight runway behind it and requires turn-into-ramp momentum preservation (needs engine-characterization spike before coding). One Pokéball at (57, 53) still puzzle-gated — skip unless easy.
...
- **Next**: Pick up obj:3 Pokéball at (33, 8) from session42 save (reachable via FAR_SHORT). For BUG-048 Gap 2, run an engine-characterization spike across turn-into-ramp regimes (hold east-south-west into `bike_ramp_W` from various runway lengths + gear) before touching BFS momentum-state. Fix BUG-047 (repel-expiration `KeyError` in navigate_to). After that, return to Route 206 (310, 608), clear the reachable under-bridge Pokéballs + Berry Soils, south through the gate house to Route 207, clear Psychic Arianna, enter Mt. Coronet.

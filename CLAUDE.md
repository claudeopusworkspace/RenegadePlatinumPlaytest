...
- **Wayward Cave status**: Main branch complete (trainers + Mira). B1F east chamber: obj:1 (Grip Claw) and obj:2 (Max Ether) picked up session 41. Session 42: slope-climb bug (wrong gear address/encoding) fixed; we can now reach the level-2 bridge row cleanly. Session 43: row-6 ramp chain solved — added BFS ramp+3 edge (emitted when ramp+4 is blocked by a same-dir ramp/wall/NPC; engine auto-truncates). `navigate_to(32, 8)` from session42 save now plans a clean 6-step path. obj:3 Pokéball (33, 8) interaction tile reachable; warp:0 to map 284 (43, 38) still behind BUG-046 (ground-level connectivity in east chamber). One Pokéball at (57, 53) still puzzle-gated — skip unless easy.
...
- **Next**: Pick up obj:3 Pokéball at (33, 8) from session42 save now that ramp+3 BFS is wired. Then tackle BUG-046 (slope+ramp runway across elevation transitions) to reach warp:0. Fix BUG-047 (repel-expiration `KeyError` in navigate_to). After that, return to Route 206 (310, 608), clear the reachable under-bridge Pokéballs + Berry Soils, south through the gate house to Route 207, clear Psychic Arianna, enter Mt. Coronet.

...
- **Wayward Cave status**: Main branch complete (trainers + Mira). B1F east chamber: obj:1 (Grip Claw), obj:2 (Max Ether), and **obj:3 (Rare Candy, session 47)** all picked up. Session 42: slope-climb bug (wrong gear address/encoding) fixed. Session 43: row-6 ramp chain solved — BFS ramp+3 edge + 3D-BFS momentum-preservation. Session 45: **BUG-048 Gap 1**. Session 47: **BUG-048 Gap 2** — direction-agnostic fast-bike momentum. **Session 50**: BFS planner reaches warp:0 from any starting tile — fixed three stacked planner bugs. **Session 51**: `_bike_ramp_segment` now terminates after the last ramp's landing (was: walked the entire remaining path into one mega-segment). `navigate_to(x=30, y=17)` from `session31_wayward_cave_bike_ramps` reaches the post-chain landing cleanly; end-to-end `navigate_to(poi=warp:0)` still bumps wild encounters along the 132-tile path (Repel-on-bike timing under investigation). One Pokéball at (57, 53) still puzzle-gated.
...
- **Next**: investigate Repel-on-bike timing — even with an active Repel, the 132-tile session-31 → warp:0 ride provoked 10 wild encounters before a Mean-Look Zubat halted the trip. Once that's understood (or a different approach taken — e.g. flying to a closer warp), session 31 → warp:0 should run end-to-end. Otherwise: return to Route 206 (310, 608), clear the reachable under-bridge Pokéballs + Berry Soils, south through the gate house to Route 207, clear Psychic Arianna, enter Mt. Coronet.

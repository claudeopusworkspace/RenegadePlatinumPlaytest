...
| `interact_with(object_index, x, y, flee_encounters)` | Navigate to a map object/NPC by index OR static tile by (x,y) and interact. Handles adjacent tiles, counter NPCs, facing, and dialogue. **Auto-advances** through full multi-page dialogue (chains into `advance_dialogue`). Detects trainer-spotted interruptions and checks for battle transitions post-dialogue. **Sign overlay support**: signpost text (board messages that bypass msgBox) is captured via memory scan and dismissed automatically — returns `sign_overlay: true`. **`flee_encounters=True`**: auto-flees wild battles encountered during the walk to the target, then re-navigates from current position to complete the interaction. |
...
| `battle_turn(move_index, switch_to, run, force)` | Full automated turn: FIGHT + move, POKEMON + switch, or RUN to flee. **Type effectiveness guardrail**: checks move type vs target types before executing. Returns `EFFECTIVENESS_WARNING` if move is immune or NVE — call again with `force=True` to proceed. **Accuracy awareness**: when active Pokemon has Acc stages <= -2, returns `accuracy_warning` with hit rate % and suggestion to switch. Returns battle log + state + trimmed battle summary (species, level, hp, types, status, stages, moves name+pp; enemy gets ability+item). |
| `throw_ball` | Throw a Poké Ball in wild battle: BAG + ball select + USE + catch result |
| `use_battle_item(item_name, party_slot, target)` | Use an item from the bag during battle. Auto-detects item category from ROM data: **healing** (Potion, Antidote, Revive — requires `party_slot` 0-5), **stat booster** (X Attack, Guard Spec — auto-applies in singles, `target` 0/1 for doubles), **escape** (Poke Doll — auto-flees). Navigates battle bag UI (4 pockets, 6 items/page, touch-based). Rejects Poke Balls (use `throw_ball`). Verifies via HP change (`read_battle`), not bag qty (stale during battle). Waits for action prompt via `_wait_for_action_prompt`. |
...
- **Badges**: 2 (Coal Badge, **Forest Badge** — Gardenia defeated)
- **Location**: Eterna City Pokemon Center. Save state: `eterna_city_post_gardenia_team_updated`.
- **Luxray** Lv30 — Jolly (+Spe/-SpA), Guts. Held: Scope Lens. Moves: Spark, Bite, Howl, Ice Fang. IVs: 13/11/23/21/6/15.
- **Grotle** Lv24 — Naughty (+Atk/-SpD), Overgrow. Held: Muscle Band. Moves: Bulldoze, Cut, Bullet Seed, Razor Leaf. IVs: 23/9/5/6/25/9.
- **Prinplup** Lv25 — Lax (+Def/-SpD), Vital Spirit. No held item. Moves: Metal Claw, Growl, Bubble Beam, Icy Wind. IVs: 30/14/7/4/23/25. *(Clutch closer vs Gardenia — Vital Spirit blocked Breloom's Spore, Icy Wind finished the fight.)*
- **Machop** Lv25 — Brave (+Atk/-Spe), No Guard. No held item. Moves: Low Kick, Brick Break, Return, Knock Off. IVs: 4/23/26/6/31/7. *(Placeholder — will be replaced by a Flying type when found.)*
- **Monferno** Lv26 — Careful (+SpD/-SpA), Iron Fist. Held: **Charcoal**. Moves: Low Kick, Mach Punch, Flame Wheel, Taunt. IVs: 6/23/24/2/20/29. *(Evolved from Chimchar Lv14. Consolidates Fire+Fighting roles. Taunt shuts down status-spam teams.)*
- **Swinub** ✨ Lv20 — Timid (+Spe/-Atk), Thick Fat. Held: **Exp. Share**. Moves: Powder Snow, Ice Shard, Bulldoze, Endure. IVs: 23/**31**/25/24/10/22. *(Shiny! Perfect Atk IV. Future Mamoswine.)*
- **Box 1**: Bulbasaur Lv5 (Docile, Chlorophyll, Miracle Seed), Swinub Lv18 (Serious, Snow Cloak — catch #2), Squirtle Lv5 (Gentle, Mystic Water), Eevee Lv12 (Gentle, Run Away), Swinub Lv19 (Gentle, Thick Fat — original catch #1), **Charmeleon Lv26** (Hardy, Blaze — benched for team comp, 0 Def/1 Spe IVs).
- **Key items**: Poke Ball x25, Antidote x3, Parlyz Heal x6, Super Potion x5, Awakening x3, Honey x10, Repel x8, Silk Scarf, Magnet, Soothe Bell, Sun Stone, Never-Melt Ice, TM58 Endure, TM Stealth Rock, TM34 Shock Wave, TM65 Shadow Claw, TM46 Thief, TM69 Rock Polish, **TM86 Grass Knot**, Fashion Case, Bicycle, Poke Radar, Town Map, Vs. Recorder, Poketch, Oval Stone, Fire Stone, HM01 Cut, HM Rock Smash.
- **Defeated trainers**: Youngster Tristan (Route 202), Youngster Logan (Route 202), Reporter Kayla (Jubilife Pokemon Center), Rival WOJ (Route 203), Youngster D (Route 203 double battle), Youngster Sebastian (Route 203), Lass Kaitlin (Route 203), Lass Madeline (Route 203), Camper Curtis (Oreburgh Gate), Picnicker Diana (Oreburgh Gate), Youngster Jonathon (Oreburgh Gym), Youngster Darius (Oreburgh Gym), **Gym Leader Roark** (6-0 sweep), 2x Team Galactic Grunts (Jubilife tag battle with Dawn), Lass Sarah (Route 204), Aroma Lady Taylor (Route 204), Bug Catcher Brandon (Route 204), Twins Liv & Liz (Route 204), Camper Jacob (Route 205), Galactic Grunt (Windworks exterior), 2x Galactic Grunts (Floaroma Meadow double battle), 2x Galactic Grunts (Windworks interior), **Commander Mars**, Hiker Daniel (Route 205), Aroma Lady Elizabeth (Route 205), Camper Zackary (Route 205), Hiker Nicholas (Route 205), Battle Girl Kelsey (Route 205), Picnicker Karina (Route 205), **Cheryl** (Eterna Forest — pre-join battle), Bug Catcher Jack (Eterna Forest), Lass Briana (Eterna Forest), Psychic Lindsey (Eterna Forest), Psychic Elijah (Eterna Forest), Hiker Louis (Route 211), Ace Trainer Laura (Route 216), Skier Edward (Route 216), Bird Keeper Katherine & Ruin Maniac Harry (Route 211 east — double battle), Lass Caroline (Eterna Gym), Aroma Lady Jenna (Eterna Gym), Aroma Lady Angela (Eterna Gym), **Gym Leader Gardenia** (6-5 clutch victory).
...
- **Gardenia's full team** (for reference): Bellossom Lv25 (Wide Lens, Chlorophyll — Stun Spore/Teeter Dance/Dazzling Gleam/Grass Knot), Tangela Lv25 (Coba Berry, Chlorophyll — Ancient Power/Shock Wave/Stun Spore/Grass Knot), Roserade Lv26 (Sitrus Berry, **Technician** — Magical Leaf/Sludge/Dazzling Gleam/Extrasensory), Grotle Lv25 (Leftovers, Overgrow — Seed Bomb/Protect/Leech Seed/Bulldoze), Cherrim Lv25 (Focus Sash, Flower Gift — Sunny Day/Weather Ball/Morning Sun/Grass Knot), Breloom Lv25 (Muscle Band, **Technician** — Bullet Seed/Mach Punch/Thunder Punch/Spore).
- **Story progress**: Beat Roark → Coal Badge. Cleared Route 204 north. Arrived Floaroma Town. Cleared Valley Windworks storyline. Looker: Team Galactic hideout is in Eterna City. Completed Route 205 north (upper path). Entered Eterna Forest, defeated Cheryl to prove strength, she joined as tag battle partner. Traversed Eterna Forest with Cheryl. Cheryl gave TM27 (Return) at exit. Crossed Route 205 bridge to Eterna City. Team Galactic grunts spotted in Eterna City. Explored Eterna City: triggered WOJ+Cyrus statue cutscene, met Cynthia (got HM01 Cut), bought herbs at Herb Shop, explored Route 211 and Mt. Coronet tunnel. Gardenia is NOT at gym — went to Route 216 (Renegade Platinum change). Galactic building requires Forest Badge. Traversed Mt. Coronet (Route 211 entrance → map 218 → map 219 → map 217 → Route 216 exit). Reached Route 216. Defeated Ace Trainer Laura and Skier Edward. Found Snowbound Lodge. **Found Gardenia** at Snowbound Lodge — she's returning to Eterna Gym. **Caught shiny Swinub** (perfect Atk IV). Returned to Eterna City. Defeated all 3 gym trainers. Scouted Gardenia (wiped to Ancient Power Tangela). Grind session: Luxio→Luxray Lv30, Chimchar→Monferno Lv25. **Beat Gardenia** on rematch (6-5 clutch, Prinplup Vital Spirit blocked Breloom's Spore for the win). Got Forest Badge + TM86 Grass Knot. Restructured team: benched Charmeleon for Swinub ✨, Charcoal to Monferno.
- **Next**: Team Galactic Eterna Building (now accessible with Forest Badge). Keep an eye out for a good Flying type to replace Machop. Shroomish on Route 203 wants an Oran Berry (come back later).

...
| `view_map` | ASCII map with terrain, player position, NPCs, and warp destinations (from ROM zone_event data). Warp coordinates can be passed directly to `navigate_to`. **Elevation-aware**: on 3D maps, passable tiles show height level numbers (0-9), ramps show `\ /`, bridges show `n*`, with an elevation summary listing all levels and the player's current height. Flat maps render unchanged. Uses BDHC data from ROM land_data files. |

...
- **Wayward Cave status**: Main branch complete (trainers + Mira). B1F east chamber: obj:1 (Grip Claw) and obj:2 (Max Ether) picked up session 41. Session 42: slope-climb bug (wrong gear address/encoding) fixed; we can now reach the level-2 bridge row cleanly. obj:3 Pokéball (33, 8) and warp:0 to map 284 (43, 38) still unreachable pending BUG-046 (ground-level connectivity in east chamber). Row-6 ramp chain puzzle wants a mid-range ramp jump (land 3 past ramp) that our BFS doesn't yet model — saved `session42_wayward_b1f_first_ramp_approach` for next session's spike. One Pokéball at (57, 53) still puzzle-gated — skip unless easy.
...
- **Next**: Spike mid-range bike-ramp jump distance (ramp+2 / ramp+3 from 1/2-tile runways), add those BFS edges, then grab the row-6 chain Pokéball. Fix BUG-046 to reach obj:3 + warp:0. Fix BUG-047 (repel-expiration `KeyError` in navigate_to). After that, return to Route 206 (310, 608), clear the reachable under-bridge Pokéballs + Berry Soils, south through the gate house to Route 207, clear Psychic Arianna, enter Mt. Coronet.

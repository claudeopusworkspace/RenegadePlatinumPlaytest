...
- **Wayward Cave status**: Main branch complete (trainers + Mira). B1F east chamber: obj:1 (Grip Claw) and obj:2 (Max Ether) picked up session 41. Session 42: slope-climb bug (wrong gear address/encoding) fixed; we can now reach the level-2 bridge row cleanly. Session 43: row-6 ramp chain solved — BFS ramp+3 edge (emitted when ramp+4 is blocked by a same-dir ramp/wall/NPC; engine auto-truncates), plus 3D-BFS momentum-preservation across level transitions. BUG-046 (slope+ramp across stairs from (13, 9)) also fully resolved as side-effect. obj:3 Pokéball (33, 8) reachable from session42. warp:0 to map 284 (43, 38) **still unreachable** — logged as BUG-048 (different failure mode than BUG-046; BFS says "reachable terrain but all paths blocked"). One Pokéball at (57, 53) still puzzle-gated — skip unless easy.
...
- **Next**: Pick up obj:3 Pokéball at (33, 8) from session42 save now that ramp+3 BFS is wired. Investigate BUG-048 (warp:0 reachability — likely surf/obstacle rather than momentum). Fix BUG-047 (repel-expiration `KeyError` in navigate_to). After that, return to Route 206 (310, 608), clear the reachable under-bridge Pokéballs + Berry Soils, south through the gate house to Route 207, clear Psychic Arianna, enter Mt. Coronet.

...
| `session28_wayward_east_wing_mid_sweep` | Wayward Cave main map 284 at (73, 29), Monferno leads, Mira at (72, 29). 7 trainers cleared session 28 (Hiker Reginald/Lorenzo, Youngster Wayne, Lass Cassidy, Picnicker Tori/Ana, Camper Diego), TM32 + Focus Band + Dusk Stone collected. Party all full HP. Party order: Monferno Lv28, Luxray Lv34, Prinplup Lv26, Grotle Lv25, Swinub ✨ Lv28, Togepi Lv1. Next: Camper at (77, 30) → Collector/Ruin Maniac duo at (91-93, 48) → Pokéball (72, 11). Mira's lost item still not found. |
| `session29_exited_wayward_under_bridge` | Wayward Cave interior (map 284 D21R0101) at (42, 53) — despite the name, the emulator renderer glitched at session 29 end and the actual save lands inside the cave antechamber, not on Route 206. warp:1 one tile west exits back to Route 206. Superseded for playthrough use by `session30_route206_under_bridge`. |
| `session30_route206_under_bridge` | Route 206 (310, 608) on foot, under east side of Cycling Road bridge, 1 tile south of the cave's east warp (warp:8 at 310, 607). Wayward Cave COMPLETE: all 10 trainers defeated sessions 28+29, Mira's quest resolved (Crimson Ribbon at (72, 11) → TM85 Dazzling Gleam reward), Mira departed. Party: Monferno Lv29, Luxray Lv34, Prinplup Lv26, Grotle Lv25, Swinub ✨ Lv28, **Togepi Lv9**. Full HP. Created session 30 after navigating through warp:1 to actually be on Route 206. Also the repro save for `TestBug038UnderBridgeReachability` — pins the view_map 3D elevation fix so bridge Cyclists obj:2/3/4 stay unreachable and the under-bridge Wayward Cave warp:7 at (299, 611) stays reachable. Next: clear undefeated Cyclist obj:4 + pick up Pokéballs + south through gate to Route 207. |
| `session31_wayward_cave_bike_ramps` | Wayward Cave B1F map 285 (D21R0102) at (7, 22) facing up, after climbing both N-S bike slopes from the west secret entrance. Party unchanged from `session30_route206_under_bridge` (6 full HP, 2 badges, Togepi Lv9). Session 31 diagnostic save — no progression, no trainers, no items. **BUG-043 repro:** east-chamber POIs (Pokéballs at (22, 9), (31, 16), (33, 8) + warp:0 to map 284 at (43, 38)) remain `unreachable_interactibles` even after session-31b's BUG-042 ramp edges, because the fast-gear bike ramp jump is 3 tiles past entry (not 2). **BUG-044 repro:** `navigate_to(x=7, y=25)` from (7, 36) auto-traverses the slope but returned `start`/`path`/`final` are misreported (overshoots target by 3). Reload before trying new ramp/slope BFS code. |
| `session32_wayward_b1f_ramp_slope_stairs` | Wayward Cave B1F map 285 (D21R0102) at (13, 9) facing right, in the mid-chamber between the N-S bike slope (x=7, rows 8-9) and the first E-bound bike ramp chain (x=15, rows 9-10). Party unchanged from session 31 (2 badges, full HP, Togepi Lv9). **New pickups**: obj:1 → Grip Claw, obj:2 → Max Ether (both obj:1 and obj:2 opened up after the session-35→40 ramp/slope/bridge work). 2 Repels used getting here (4 remaining). **BUG-046 repro:** `navigate_to(x=28, y=6)` from (13, 9) errors with `"No reasonable path at your current elevation (level 0)"` — originally suspected to be a slope+ramp-runway issue, but session-42's BDHC dump confirmed it's a ground-level-connectivity problem: (13, 9), (23, 9), and (33, 8) are all level 0 but no walkable level-0 path connects the east chamber to the west corridor. Remaining unreachable POIs: obj:3 Pokéball (33, 8), warp:0 to map 284 (43, 38). |
| `session42_wayward_b1f_first_ramp_approach` | Wayward Cave B1F map 285 (D21R0102) at (25, 6) facing right, on the level-2 bridge row just east of the first E-bound bike ramp `>` at (26, 6). Party unchanged from session 32. **Session-42 gear fix:** slope climb now works with correct BIKE_GEAR_STATE_ADDR (PPB+0x8c) + inverted encoding. 3 Repels remaining (one active). Save point used by the BUG-048 Gap 2 work in sessions 45-47. |
| `session47_wayward_b1f_post_obj3_rare_candy` | **CURRENT (playthrough)** — Wayward Cave B1F map 285 (D21R0102) at (33, 7) facing down, just past the row-6 ramp chain. obj:3 Pokéball collected (Rare Candy → Medicine pocket). Party unchanged from session 42, 3 Repels remaining (one active). **Session 47:** BUG-048 Gap 2 fixed end-to-end — direction-agnostic fast-bike momentum (BFS state simplified to `(x, y, m)`; executor drives multi-direction segments via `drive_bike_subsegments` using `advance_frames_until + final_buttons` handoff). `navigate_to(poi=obj:3)` from session42 plans `right x4 → down → right` and drives the turn-into-ramp segment cleanly. warp:0 at (43, 38) is still unreachable (Manhattan 41) — different chamber geometry south of here, separate problem. |
| `bug_nav_repel_expired_move_keyerror` | **BUG-047 regression save (FIXED session 46).** Wayward Cave B1F map 285 at (16, 6) facing right, on the level-2 bridge row about to cross eastward. Repel is about to expire during traversal. Post-fix: `navigate_to(x=25, y=6, flee_encounters=True)` completes cleanly in one call to (25, 6), `steps: 9`, `path: "right x9 -> right x3"`. Kept as the regression save pinned by `tests/test_qa_bug047_nav_repel_expiration.py`. |
| `bug048_wayward_b1f_east_chamber_via_chain_through` | **BUG-048 Gap-1 verification save.** Wayward Cave B1F map 285 at (42, 6), post-CHAIN_THROUGH landing after the row-6 ramp chain. Shows the east chamber is reachable via the engine's hold-through chain behavior, even though straight-line BFS couldn't reach it pre-fix. Used to pin the chain-landing arithmetic (approach + 2×JUMP_TILES) and document the remaining Gap 2 (interior ramp needs turn-into-momentum). |
...
| `bug_reorder_party_fails_silently_with_field_move` | Overworld with Grotle at slot 0 (knows Cut) and Monferno at slot 3. **BUG-033 repro** — `reorder_party(0, 3)` used to report `success: true` while the party was unchanged: Cut added a row to the Pokemon sub-menu above Switch, and the old hardcoded `down x1` cursor landed on Cut instead. Fixed in session 27 via the `renegade_mcp.party_submenu` helper + post-swap species-ID verification; used by `TestReorderParty::test_swap_when_source_knows_field_move`. |
| `bug_togepi_cascade_levelup` | Wayward Cave east wing map 284 at (91, 47), Mira at (92, 47), Exp. Share just moved from Swinub to Togepi (slot 5, Lv1). 2 steps from Ruin Maniac Gerald at (93, 48). **BUG-038 repro** — call `navigate_to(poi="obj:14", flee_encounters=True)` and fight through Gerald's 3-mon team (Cubone Lv25 → Probopass Lv25; final 3rd fight triggers 529 XP into Togepi, jumping Lv7→Lv9 in one XP block with chained move-learn prompts). Observed issues: (1) `battle_turn(forget_move=0)` at the Lv2 Metronome learn advances straight to SWITCH_PROMPT without surfacing the "learned Metronome" confirmation (learn did commit in memory, but log skipped); (2) a later single XP payout chains multiple level-ups (Lv7→Lv8→Lv9) with move-learn prompts that `forget_move=-1` doesn't cleanly dismiss — tool returns `final_state=NO_TEXT` while the "Make it forget another move?" box is still on-screen; (3) mid-battle `read_party` returned stale save-block data (Togepi Lv1 with Growl still in slot 0) during the transition — fresh read post-battle showed correct Lv9 with Metronome. Good stress-test for multi-level-up + move-learn + stale-state handling. |
| `bug_bike_bridge_unknown` | Wayward Cave 2F east wing map 285 at (22, 13), **on foot**. Captured at the next obstacle after the 4-ramp chain puzzle (session 39) — an apparent "bicycle bridge" feature not yet understood or implemented. Intended for a dedicated future session: observe engine behavior when stepping north/east onto the bridge tile, classify the tile-behavior byte, and extend the pathfinder/executor accordingly. No regression code lives against this save state yet. |

...
| `session31_wayward_cave_bike_ramps` | Wayward Cave B1F map 285 (D21R0102) at (7, 22) facing up, on bike, after climbing both N-S bike slopes from the west secret entrance. Party unchanged from `session30_route206_under_bridge` (6 full HP, 2 badges, Togepi Lv9). Session 31 diagnostic save — no progression, no trainers, no items. **Session-50 update:** `view_map` now lists all 5 interactibles as reachable (obj:1@56, obj:2@13, obj:3@83, warp:0@132, warp:1@41) after the always-NEAR + multi-level-transition + depth-15 fixes. End-to-end `navigate_to(poi=warp:0)` still stalls at (25, 17) where a bike-ramp segment lands the player on a chain ramp; tracked as the executor-side followup. Older repro tags (BUG-043, BUG-044) resolved upstream — keeping this save as the canonical "BFS planner stress test" for the south-corridor → upper-chamber path. |
| `session32_wayward_b1f_ramp_slope_stairs` | Wayward Cave B1F map 285 (D21R0102) at (13, 9) facing right, in the mid-chamber between the N-S bike slope (x=7, rows 8-9) and the first E-bound bike ramp chain (x=15, rows 9-10). Party unchanged from session 31 (2 badges, full HP, Togepi Lv9). **New pickups**: obj:1 → Grip Claw, obj:2 → Max Ether (both obj:1 and obj:2 opened up after the session-35→40 ramp/slope/bridge work). 2 Repels used getting here (4 remaining). Post-session-50: warp:0 reach@84, obj:3 reach@35 (no longer in `unreachable_interactibles`). |
| `session42_wayward_b1f_first_ramp_approach` | Wayward Cave B1F map 285 (D21R0102) at (25, 6) facing right, on the level-2 bridge row just east of the first E-bound bike ramp `>` at (26, 6). Party unchanged from session 32. **Session-42 gear fix:** slope climb now works with correct BIKE_GEAR_STATE_ADDR (PPB+0x8c) + inverted encoding. 3 Repels remaining (one active). Save point used by the BUG-048 Gap 2 work in sessions 45-47. Post-session-50: warp:0 reach@55. |
| `session47_wayward_b1f_post_obj3_rare_candy` | **CURRENT (playthrough)** — Wayward Cave B1F map 285 (D21R0102) at (33, 7) facing down, just past the row-6 ramp chain. obj:3 Pokéball collected (Rare Candy → Medicine pocket). Party unchanged from session 42, 3 Repels remaining (one active). **Session 47:** BUG-048 Gap 2 fixed end-to-end — direction-agnostic fast-bike momentum (BFS state simplified to `(x, y, m)`; executor drives multi-direction segments via `drive_bike_subsegments` using `advance_frames_until + final_buttons` handoff). Post-session-50: warp:0 reach@55, no `unreachable_interactibles` (the prior "warp:0 still unreachable (Manhattan 41)" claim was stale doc, not actual BFS state). |

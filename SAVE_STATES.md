...
| `session42_wayward_b1f_first_ramp_approach` | **CURRENT (playthrough)** — Wayward Cave B1F map 285 (D21R0102) at (25, 6) facing right, on the level-2 bridge row just east of the first E-bound bike ramp `>` at (26, 6). Party unchanged from session 32. **Session-42 gear fix:** slope climb now works with correct BIKE_GEAR_STATE_ADDR (PPB+0x8c) + inverted encoding. 3 Repels remaining (one active). Save point for next session's mid-range ramp jump spike — the row-6 ramp chain Pokéball wants a 3-tile jump past the first ramp, which our BFS doesn't model (FAR=4, NEAR=1; mid-range unmodeled). |
| `bug_nav_repel_expired_move_keyerror` | **BUG-047 repro.** Wayward Cave B1F map 285 at (16, 6) facing right, on the level-2 bridge row about to cross eastward. Repel is about to expire during traversal. Repro: load, `navigate_to(x=25, y=6, flee_encounters=True)` — returns `{"error": "Error executing tool navigate_to: 'move'"}` instead of cleanly reporting the repel-expiration / resulting wild encounter. Root cause: `renegade_mcp/navigation.py:1751/1755` accesses `ob["move"]` unconditionally when formatting the obstacle-choice prompt, and some obstacle dict in the flow lacks that key. |
